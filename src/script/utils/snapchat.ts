...
export function getMediaContentProto() {
  const webpackRequire = getSnapchatWebpackRequire();
  const someModuleId = getSnapchatWebpackModuleId((module) => module.includes('userBotResponseMetadata'));
  if (webpackRequire == null || someModuleId == null) {
    return null;
  }

  const module = webpackRequire(someModuleId) as Record<string, any>;
  const mediaContentProto = Object.values(module).find((value) => value.encode != null && value.decode != null);

  return mediaContentProto;
}
