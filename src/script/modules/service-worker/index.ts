...
(async () => {
  ...
  const scripts = await import('./scripts/*.ts');
  let importScriptString = '';
  for (const script of scripts.default) {
    importScriptString += `${script.default.toString()};${script.default.name}();`;
  }
